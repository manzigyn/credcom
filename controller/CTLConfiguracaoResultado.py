...
def obter(contratante: entCon.Contratante) -> entConfig.ConfiguracaoResultado:
    ...
    configuracaoResultado.sms = int(df_configuracao["ConfValor"].unique()) if not df_configuracao.empty else 0
    
    df_configuracao = df_configuracoes[df_configuracoes["ConfChave"] == enConfig.EnumConfiguracaoResultado.LIGACAO]
    configuracaoResultado.ligacao = int(df_configuracao["ConfValor"].unique()) if not df_configuracao.empty else 0
